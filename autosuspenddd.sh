...
info "Mengunduh autosuspendfile.zip..."

if ! wget -q --show-progress \
"https://raw.githubusercontent.com/manziero/autosuspend-LXJROFFC/main/autosuspendfile.zip" \
-O autosuspendfile.zip; then
    error "Gagal mengunduh autosuspendfile.zip."
    exit 1
fi

if [[ ! -s autosuspendfile.zip ]]; then
    error "autosuspendfile.zip kosong atau tidak valid."
    exit 1
fi

success "autosuspendfile.zip berhasil diunduh."

info "Mengekstrak autosuspendfile.zip..."

mkdir -p extracted

if ! unzip -oq autosuspendfile.zip -d extracted; then
    error "Gagal mengekstrak autosuspendfile.zip."
    exit 1
fi
